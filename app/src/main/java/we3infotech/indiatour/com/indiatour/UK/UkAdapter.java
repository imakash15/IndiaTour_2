...
public class UkAdapter extends RecyclerView.Adapter<UkAdapter.MyViewHolder> {
    private Context co;
    private ArrayList<UkList> ukLists;
    private UkAdapter.OnItemClickListener mListener;

    public UkAdapter(Context context, ArrayList<UkList> ukListArrayList) {
        this.co = context;
        this.ukLists = ukListArrayList;
    }

    @Override
    public UkAdapter.MyViewHolder  onCreateViewHolder(ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(co).inflate(R.layout.model, parent, false);
        return new UkAdapter.MyViewHolder(view);
    }

    @Override
    public void onBindViewHolder(UkAdapter.MyViewHolder holder, int position) {
        holder.tvName.setText(ukLists.get(position).getName());
        holder.tvPrice.setText(ukLists.get(position).getPrice());
        holder.desctxt.setText(ukLists.get(position).getDescription());
        holder.minorder.setText(ukLists.get(position).getMinOrder());

        Glide.with(co)
                .load(ukLists.get(position).getImageUrl())
...
//        holder.btnCallNow.setOnClickListener(new NorthAdapter.OnButtonClick(position));
    ...
    public void setOnClickListner(UkAdapter.OnItemClickListener onClickListner) {
