...
public class HpAdapter extends RecyclerView.Adapter<HpAdapter.MyViewHolder> {
    private Context co;
    private ArrayList<HpList> hpLists;
    private HpAdapter.OnItemClickListener mListener;

    public HpAdapter(Context context, ArrayList<HpList> hpListArrayList) {
        this.co = context;
        this.hpLists = hpListArrayList;
    }

    @Override
    public HpAdapter.MyViewHolder  onCreateViewHolder(ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(co).inflate(R.layout.model, parent, false);
        return new HpAdapter.MyViewHolder(view);
    }

    @Override
    public void onBindViewHolder(HpAdapter.MyViewHolder holder, int position) {
        holder.tvName.setText(hpLists.get(position).getName());
        holder.tvPrice.setText(hpLists.get(position).getPrice());
        holder.desctxt.setText(hpLists.get(position).getDescription());
        holder.minorder.setText(hpLists.get(position).getMinOrder());

        Glide.with(co)
                .load(hpLists.get(position).getImageUrl())
...
//        holder.btnCallNow.setOnClickListener(new NorthAdapter.OnButtonClick(position));
    ...
    public void setOnClickListner(HpAdapter.OnItemClickListener onClickListner) {
