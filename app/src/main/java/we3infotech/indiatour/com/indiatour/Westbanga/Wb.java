package we3infotech.indiatour.com.indiatour.Westbanga;
...
public class Wb extends AppCompatActivity {
    ...
    private ArrayList<WbList> wbLists = new ArrayList<>();
    ...
    private void initView() {
        ...
        if (isNetworkAvailable()) {
            AsyncPost asyncPost = new AsyncPost(Wb.this, new ResponseListener() {
                @Override
                public void onResponse(String result) {
                    ...
                    try {
                        ...
                        for (int i = 0; i < jsonArray.length(); i++) {
                            ...
                            wbList.setName(e.getString("name"));
                            wbList.setDescription(e.getString("description"));
                            wbList.setPrice(e.getString("price"));
                            wbList.setImageUrl(e.getString("imageUrl"));
                            wbList.setMinOrder(e.getString("minOrder"));
                            wbList.setMenu(e.getString("menu"));
                            wbLists.add(wbList);
                        }
                    ...
                    wbAdapter = new WbAdapter(getApplication(), wbLists);
                    rvList.setAdapter(wbAdapter);
                    wbAdapter.setOnClickListner(new WbAdapter.OnItemClickListener() {
                        @Override
                        public void OnItemClick(int position) {
                            Intent intent = new Intent(Wb.this,OnItemClic.class);
                            intent.putExtra("id",wbLists.get(position).getPrice());
                            intent.putExtra("name",wbLists.get(position).getName());
                            intent.putExtra("desc",wbLists.get(position).getDescription());
                            intent.putExtra("menu",wbLists.get(position).getMenu());
                            intent.putExtra("imageurl",wbLists.get(position).getImageUrl());
            ...
            asyncPost.execute("https://we3infotech.com/IndiaTour/wb.php");
        } /*else {
