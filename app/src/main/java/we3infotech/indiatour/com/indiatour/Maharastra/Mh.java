...
public class Mh extends AppCompatActivity {
    ...
    private ArrayList<MhList> mhLists = new ArrayList<>();
    ...
    private void initView() {
        ...
        if (isNetworkAvailable()) {
            AsyncPost asyncPost = new AsyncPost(Mh.this, new ResponseListener() {
                @Override
                public void onResponse(String result) {
                    ...
                    try {
                        ...
                        for (int i = 0; i < jsonArray.length(); i++) {
                            ...
                            MhList mhList = new MhList();
                            mhList.setName(e.getString("name"));
                            mhList.setDescription(e.getString("description"));
                            mhList.setPrice(e.getString("price"));
                            mhList.setImageUrl(e.getString("imageUrl"));
                            mhList.setMinOrder(e.getString("minOrder"));
                            mhList.setMenu(e.getString("menu"));
                            mhLists.add(mhList);
                        }
                    ...
                    mhAdapter = new MhAdapter(getApplication(), mhLists);
                    rvList.setAdapter(mhAdapter);
                    mhAdapter.setOnClickListner(new MhAdapter.OnItemClickListener() {
                        @Override
                        public void OnItemClick(int position) {
                            Intent intent = new Intent(Mh.this,OnItemClic.class);
                            intent.putExtra("id",mhLists.get(position).getPrice());
                            intent.putExtra("name",mhLists.get(position).getName());
                            intent.putExtra("desc",mhLists.get(position).getDescription());
                            intent.putExtra("menu",mhLists.get(position).getMenu());
                            intent.putExtra("imageurl",mhLists.get(position).getImageUrl());
            ...
            asyncPost.execute("https://we3infotech.com/IndiaTour/mh.php");
        } /*else {
